...
pub fn render_modal(f: &mut Frame, app: &App) {
    ...
    // Modal dimensions: fixed width, height fits list + chrome
    let modal_height = (LIST_HEIGHT + 8) as u16; // list + title + footer + borders + outer box
...
fn render_list(f: &mut Frame, app: &App, area: ratatui::layout::Rect) {
    let filter = &app.account_filter;
    let cursor = app.account_filter_cursor;
    let scroll = app.account_filter_scroll;

    // Count how many are checked for the title indicator
    ...
    if filter.is_empty() {
        let para = Paragraph::new(Line::from(Span::styled(
            "  No Income or Expenses accounts found.",
